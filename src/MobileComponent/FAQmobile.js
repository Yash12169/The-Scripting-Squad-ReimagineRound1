...
import '../stylesMobile/FAQmobile.css';
    ...
    return (
        <div className={'flex flex-col z-[150]'}>
            <div className='mobile-faq-container'>
                <div className='mobile-faq-head montserrat-reg'>
                    <p>Your questions, answered</p>
                </div>
                <div className='mobile-faq-content'>
                    {questions.map((question) => (
                        <div key={question.id} className='mobile-faq-question montserrat-reg'>
                            <div className='mobile-ques-box'>
                                <p className=''>{question.question}</p>
                                <img
                                    onClick={() => handleImageClick(question.id)}
                                    className='cursor-pointer mobile-faq-close-img'
                                    style={{ transform: `rotate(${rotation[question.id] || 0}deg)` }}
                                    src={plus}
                                    alt='plus'
                                />
                            </div>
                            <div className={`mobile-faq-answer ${openQuesIds.includes(question.id) ? 'open' : ''}`}>
                                <p className='text-[15px] poppins-regular mt-[20px] mb-[20px]'>{question.answer}</p>
                            </div>
                        </div>
                    ))}
                </div>
                <div className={'mb-[100px] mt-[50px] ml-[30px]'}>
                    <p className={'mobile-link-faq montserrat-reg'}>Can't find what you're looking for?</p>
                    <p className={'mobile-link-faq montserrat-reg'}>Visit our <span className={'mobile-underline-faq'}>FAQs page</span>
                    </p>
                </div>
            </div>

            <div className={'mobile-faq-para montserrat-reg'}>
            ...
            <div className={'mobile-caution-cont montserrat-reg'}>
                <div className={'flex mobile-caution'}>
                    <img className={'w-[24px] h-[24px]'} src={sound} />
                    <p className={'montserrat-reg caution-text-mobile'}>Caution Notice</p>
                </div>
                <div className={'fraud-mobile'}>
                    <p className={'montserrat-reg  text-[#F8F8FF]'}>Beware of scammers in Ather's clothing. Don't get tricked into frauds.</p>
                </div>
            </div>
        </div>
    );
...
const questions = [
    ...
    {
        id: 3,
        question: "Do I need a Driver's License, Helmet and Registration to use an electric scooter?",
        answer: "An Ather electric scooter is subject to the same Central & State road safety laws as any other scooter. Therefore, yes, a valid driver's license, registration & helmet are absolutely mandatory for riding an Ather Electric Scooter."
    },
    {
        id: 4,
